...
def test_announcement_fetch_detail_compatibility_method_returns_enriched_item(
    ...
    monkeypatch.setattr(
        "applyhome_alert.fetcher.fetch_announcement_detail",
        lambda item: item.with_detail(detail),
    )

    enriched = announcement.fetch_detail()
